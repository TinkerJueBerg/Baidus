# -*- coding: utf-8 -*-
"""
Created on Mon Aug  5 21:39:57 2019
...
class GoogleNet(): 
    ...
    def xavier(self, channels, filter_size, name): 
        stdv = (2.0 / (filter_size*2 * channels))**0.5 
        param_attr = ParamAttr( 
            initializer=fluid.initializer.Uniform(0, stdv), 
            name=name + "_weights") 
    ...
    def generalization_para(self,input):
        
        omega = random.uniform(0.55,0.8)
        input = np.multiply(input,omega)
        return input
    
    def net(self, input, class_dim=1000): 
        
        ge_input = self.generalization_para(input)
...
def eval_all():     
    ...
    total_count = 0     
    right_count = 0     
    for img in os.listdir(eval_image_dir):
        img = custom_image_reader(eval_file_path,data_dir,train_parameters['eval_mode'])
...
StackingCsv.to_csv('Satellite666.csv',index=False,sep=',')
print("every program is over")